...
import { Search, SlidersHorizontal, X, ChevronUp, ChevronDown, Filter, List } from "lucide-react";
  ...
  const displayedRanked = useMemo(() => {
    if (searchMode === "filter" && search.trim().length >= 1) {
      ...
      return ranked.filter(
        (r) => r.country.name.toLowerCase().includes(q) || r.country.code.toLowerCase() === q,
      );
    }
    return ranked;
          ...
          <main className="flex-1 min-w-0" style={{ backgroundColor: "#0F1114" }}>
            ...
            <div className="flex items-center gap-4 md:gap-6">
              <div>
                ...
                <div style={{ fontFamily: "Geist, sans-serif", fontSize: "10px", color: "#444444", textTransform: "uppercase", letterSpacing: "1px", marginTop: "4px" }}>{t("hero.countries")}</div>
              </div>
              <div className="w-px h-6 md:h-8" style={{ backgroundColor: "#333333" }} />
              <Link to={`${langPrefix}/indicators`} style={{ textDecoration: "none" }}>
                <div>
                  ...
                  <div style={{ fontFamily: "Geist, sans-serif", fontSize: "10px", color: "#444444", textTransform: "uppercase", letterSpacing: "1px", marginTop: "4px" }}>{t("hero.indicators")}</div>
                </div>
              </Link>
              <div className="w-px h-6 md:h-8" style={{ backgroundColor: "#333333" }} />
              <Link to={`${langPrefix}/data-sources`} style={{ textDecoration: "none" }}>
                <div>
                  <div style={{ fontFamily: "IBM Plex Mono, monospace", fontSize: "18px", fontWeight: 600, color: "var(--color-accent-dim)", lineHeight: "1" }}>10</div>
                  <div style={{ fontFamily: "Geist, sans-serif", fontSize: "10px", color: "#444444", textTransform: "uppercase", letterSpacing: "1px", marginTop: "4px" }}>{t("hero.dataSources")}</div>
                </div>
              </Link>
            </div>
                </div>
                ...
                <div className="flex flex-wrap gap-2">
                  <button
                    ...
                    style={{
                      ...
                      color: ws.selectedRegions.size === 0 ? "#FFFFFF" : "#999999",
                    }}
                  >
                    {t("regions.all")}
                  </button>
                  {regions.map((r) => (
                    <button
                      key={r}
                      onClick={() => ws.setSelectedRegions((prev) => {
                      ...
                      style={{
                        ...
                        color: ws.selectedRegions.has(r) ? "#FFFFFF" : "#999999",
                      }}
                    >
                      {r}
                    </button>
